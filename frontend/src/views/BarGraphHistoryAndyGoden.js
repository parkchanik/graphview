...
import ryukimyangcsv from "./data/KBOYONG.csv";
//import typelist from "./data/KINGLIST.csv";
export class BarGraphHistoryAndyGoden extends Component {
  constructor(props) {
  ...
  SetData() {
    ...
    alldata.map(d => {
      ...
      if (d.YEAR === nownum.toString()) {
        ...
        data2.push(wjsondata);

        let ljsondata = { name: d.NAME, gubun: d.TEAM, value: d.L };
        data3.push(ljsondata);

        let ipjsondata = { name: d.NAME, gubun: d.TEAM, value: d.IP };
        data4.push(ipjsondata);
      }
    });
